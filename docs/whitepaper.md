...
These claims are supported by 1808 regression tests (including optional Hypothesis property-based tests) and by proof sketches in `docs/metatheory.md`. They are **not machine-checked proofs** in a formal proof assistant.
...
Stele has 1808 tests (1804 passing, 4 skipped without Hypothesis), organized into 44 test files:
...
Stele v1.0.0 comprises 1808 tests, 15 curated gallery examples with CI-enforced honesty labels, and zero runtime dependencies for the core verification path. It is available as a public GitHub Pages site, a single-file HTML, a standalone executable, and a local Python package.
